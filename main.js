...
class Opnsense extends utils.Adapter {
                                ...
                                controller &&
                                    controller.commands.forEach((command) => {
                                        ...
                                        this.setObjectNotExistsAsync(
                                            module.name + '.' + controller.name + '.' + command.name,
                                            {
                                                type: 'channel',
                                                common: {
                                                    name: command.name,
                                    ...
                                    case 'get':
                                        client
                                            ...
                                            .then(async (result) => {
                                                ...
                                                this.setStates(
                                                    [module.name, controller.name, command.name].join('.'),
                                                    transformed,
                                                );
                                            })
                                            .catch((reason) => {
    ...
    setStates(parentName, transformed) {
        for (const [key, value] of Object.entries(transformed)) {
            console.log(`${key}(${key.toUpperCase()}): ${value}`);
            const stateName = [parentName, key.toUpperCase()].join('.');
            if (isObject(value)) {
